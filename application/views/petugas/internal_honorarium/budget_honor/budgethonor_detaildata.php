                    ...
                    <div class="card">
                        ...
                        <div id="petugasID"><?=$this->fungsi->petugas_login()->subbidangID?></div>
                            ...
                            <div class="box">
                                <!-- /.box-header -->
                                <div class="box-body">
                                <input type="text" name="budgetDetailID" id="budgetDetailID" value="<?=$row->budgethonorNo?>">
...
$(document).ready(function(){
    ...
    function getNewOrder(){
        ...
        var id = $('#budgetDetailID').val();

        $.ajax({
            type : "GET",
            url  : "<?php echo base_url('IntHonorarium/getDetailDataBudgetHonor')?>",
            dataType : "JSON",
            data  : {id:id},
                ...
                for(i=0; i<data.length; i++){
                    html += "<tr>"+
                            ...
                            if(data[i].status == 'failed' && petugas == data[i].penerima){
                                html += "<td style='text-align:center'><a href='<?=base_url('IntHonorarium/revisiBudgetHonor/')?>"+data[i].budgetDetailID+"'><button id='btn-detail' class='btn btn-info btn-sm'>Detail</button> </td>";
                            }else{
                                html += "<td style='text-align:center'><a href='<?=base_url('IntHonorarium/konfBudgetHonor/')?>"+data[i].budgetDetailID+"'><button id='btn-detail' class='btn btn-info btn-sm'>Detail</button> </td>";
                            }
