...
$(document).ready(function(){
    ...
    function getNewOrder(){
        $.ajax({
            type : "GET",
            url  : "<?php echo base_url('Invoicing/getDataINVFRMinerba')?>",
            dataType : "JSON",
            success : function(data){
                ...
                for(i=0; i<data.length; i++){
                    ...
                    html += "<td>"+data[i].waktu+"</td>"+
                            "<td style='text-align:center'><a href='<?=base_url('order/getorderdetail/')?>"+data[i].orderID+"'><button id='btn-detail' class='btn btn-info btn-sm'>Detail</button> </a><a href='<?=base_url('viewDataInvoiceSpkIDMinerba/')?>"+data[i].spkID+"'><button class='btn btn-primary btn-sm'>konfirmasi</button></a></td>"+
                            "</tr>";
                }
