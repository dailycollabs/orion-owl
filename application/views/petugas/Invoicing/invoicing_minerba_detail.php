                ...
                <div class="container-fluid" style="max-width: 100%;">
                    ...
                    <div class="col-md-6">
                        <div class="card card-info ">
                            ...
                            <input type="hidden" name="frID" id="frID" value="<?=$row->frMinD_frID?>">
                            <table class="table">
                                <thead>
                                </thead>
                                <tbody>
                                    ...
                                    <?php if($rowInvID != null){?>
                                    <?php if($rowInv != null){?>
                                    <tr>
                                        <td>ID INV Report</td>
                                        <td id="invID"><?=$rowInv->invMinD_invID?></td>
                                    </tr>
                                    <?php } ?>
                                    <?php } ?>
                                    <tr>
...
$(document).ready(function(){
    ...
    function getInvoicingDetail(){
        ...
        $.ajax({
            type : "GET",
            url  : "<?php echo base_url('Invoicing/getDataInvoicingFRDMinerba')?>",
            dataType : "JSON",
            data:{frID:frID},
                ...
                for(i=0; i<data.length; i++){
                    ...
                    html += "<td>"+data[i].waktu+"</td>";
                            ...
                            if(data[i].status == 'failed'){
                                html +=  "<td style='text-align:center'><a href='<?=base_url('InvoicingFinalReport/revisiDraftFR/')?>"+data[i].frDetailID+"'><button class='btn btn-primary btn-sm'>Revisi</button></a></td>";
                            }else if(data[i].status == 'new' || data[i].status == 'revisi' || data[i].status == 'reject'|| data[i].status == 'success'){
                                html +=  "<td style='text-align:center'><a href='<?=base_url('InvoicingFinalReport/konfirmasiFR/')?>"+data[i].frDetailID+"'><button class='btn btn-primary btn-sm'>Konfirmasi</button></a></td>";
                            }
    ...
    function getDraftInvoicing(){
        ...
        $.ajax({
            type : "GET",
            url  : "<?php echo base_url('Invoicing/getDetailDataInvoicingMinerba')?>",
            dataType : "JSON",
            data:{invID:invID},
            success : function(data){
                ...
                for(i=0; i<data.length; i++){
                    html += "<tr>"+
                            ...
                            if(data[i].status == 'failed'){
                                ...
                                html +=  "<td style='text-align:center'><a href='<?=base_url('InvoicingDraft/konfirmasiInvMin/')?>"+data[i].invDetailID+"'><button class='btn btn-primary btn-sm'>Konfirmasi</button></a></td>";
                            }
    ...
    function getCheck(){
        ...
        $.ajax({
            type : "GET",
            url  : "<?php echo base_url('Invoicing/getCheckAllMin')?>",
            dataType : "JSON",
            data:{frID:frID},
            success : function(data){
