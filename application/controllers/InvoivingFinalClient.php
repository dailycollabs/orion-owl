...
class InvoivingFinalClient extends CI_Controller {

    function __construct(){
        parent::__construct();
        check_not_petugas();
        $this->load->model(['ClientProject_m','Client_m','InvoicingApproval_m', 'InvoiceFinalClient_m']);
        ...
        if($this->form_validation->run() == FALSE){
            ...
            if($this->fungsi->petugas_login()->bidangID == 1){
                ...
                if($this->db->affected_rows()>0){
                    ...
                    $postInv['frDetailID'] = $post['invDetailID'];
                    $postInv['status']    = 'SC1';

                    $this->InvoicingApproval_m->setInvDMarStatus($postInv);
                ...
                if($this->db->affected_rows()>0){
                    ...
                    $this->InvoicingApproval_m->setInvDMinStatus($postInv);
                    $this->ClientProject_m->setStatus($postClient); 
